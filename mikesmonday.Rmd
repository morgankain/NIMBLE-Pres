...
```{r, "MCEM"}
tempmod <-mod2$newModel()
box <- list(list(c("beta","reporting","effprop"),c(0,1)))

mod2MCEM <- buildMCEM(model=mod2, latentNodes = "I[1:10]",boxConstraints = box)
mod2MLE <- mod2MCEM()
...
inits <- list(list(
  ...
  effpropI = effpropI - 0.001,
  beta = beta + .05,
  reporting = reporting),
list(
  I = sim$I  ,
  effpropS = effpropS - 0.1,
  effpropI = effpropI + 0.002,
  beta = beta + .1,
  reporting = reporting),
list(
  I = sim$I ,
...
nimCBdata <- list(obs = sim$Iobs)
nimCBcon <- list(numobs = numobs, N = pop, i0 = 2)
